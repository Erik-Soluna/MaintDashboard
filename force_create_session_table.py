#!/usr/bin/env python3
"""
Force create django_session table via API.
"""
...
def force_create_session_table():
    """Force create the django_session table."""
    print("Force creating django_session table...")
    
    # Try to run migrations specifically for sessions app
    payload = {"command": "migrate", "app": "sessions", "fake_initial": False}
    ...
    if status_code == 200 and result.get('success'):
        ...
        # Try fake initial for sessions
        print("\nTrying fake initial for sessions...")
        payload = {"command": "migrate", "app": "sessions", "fake_initial": True}
        result, status_code = make_api_request(payload)
        
        if status_code == 200 and result.get('success'):
            print("Sessions fake initial completed successfully")
            print("Migration output:")
            print(result.get('output', 'No output'))
            return True
        else:
            print(f"Sessions fake initial failed: {result.get('error', 'Unknown error')}")
            return False

def test_login_after_session_fix():
    """Test login after session table fix."""
    print("\nTesting login after session table fix...")
    ...
    try:
        ...
        # Try login with the correct password from logs
        login_data = {
            'username': 'admin',
            'password': 'DevAdminPassword2024!',  # From logs
...
def main():
    """Main function."""
    print("Force Create Session Table Script")
    print("=" * 50)
    ...
    if force_create_session_table():
        ...
        # Test login
        if test_login_after_session_fix():

...
export default function Index() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await fetch('/core/overview/data/');
        if (!response.ok) {
  ...
  if (loading) {
    return (
      <div className="flex w-full min-h-screen items-center justify-center bg-dashboard-background">
  ...
  if (error) {
    return (
      <div className="flex w-full min-h-screen items-center justify-center bg-dashboard-background">
        <div className="text-red-500 text-2xl">Error loading dashboard: {error}</div>
      </div>
    );
  }

  const statsData = [
    { label: "Active Equipment", value: data.stats.activeEquipment.toString() },
    { label: "Pending Maintenance", value: data.stats.pendingMaintenance.toString() },
    { label: "Overdue Items", value: data.stats.overdueItems.toString() },
    { label: "Events This Week", value: data.stats.eventsThisWeek.toString() },
    { label: "Completed This Month", value: data.stats.completedThisMonth.toString() }
  ];
  ...
  return (
    <div className="flex w-full min-h-screen flex-col items-start bg-dashboard-background">
      ...
      <div className="flex min-w-full px-9 py-7 justify-center items-center gap-3 bg-dashboard-header">
        <div className="flex h-11 px-8 justify-between items-center flex-1">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="text-white text-2xl font-normal">Maintenance Dashboard</div>
          </div>
      ...
      <div className="flex min-w-full px-12 py-10 flex-col items-center gap-13 bg-dashboard-background">
        ...
        <div className="flex pb-14 justify-center items-center gap-12 self-stretch flex-wrap mt-8">
            ...
            <div className="flex flex-col items-start gap-1 flex-1 col-span-1 justify-center w-full">
              ...
              <div className="flex flex-col gap-0 flex-1 max-h-[645px] overflow-y-auto scrollbar-thin scrollbar-track-black scrollbar-thumb-gray-600 w-full">
                {data.urgentItems.length > 0 ? (
                  ...
                  <div className="text-white text-center p-4">No urgent items</div>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-col justify-center items-start gap-1 flex-1 col-span-1">
              ...
              <div className="flex flex-col gap-0 flex-1 max-h-[645px] overflow-y-auto scrollbar-thin scrollbar-track-black scrollbar-thumb-gray-600 w-full">
                {data.upcomingItems.length > 0 ? (
                  data.upcomingItems.map((item, idx) => (
                    <MaintenanceItem key={idx} {...item} />
                  ))
                ) : (
                  <div className="text-white text-center p-4">No upcoming items</div>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="grid h-[724px] min-w-[784px] gap-11 flex-1 grid-cols-3 grid-rows-2">
            {data.equipmentCards.length > 0 ? (
              data.equipmentCards.map((equipment, idx) => (
                <EquipmentCard key={idx} {...equipment} />
              ))
            ) : (
              <div className="col-span-3 row-span-2 flex items-center justify-center">
                <div className="text-white text-center">No equipment data available</div>
              </div>
            )}
          </div>
        </div>
      ...
      {/* Footer */}
      <div className="flex min-w-full px-9 py-7 justify-center items-center bg-dashboard-header">
